/********************************************************************************
 * 
 * UNIVERSAL SCRIPT TO PROVIDE COMMON ENTRYPOINTS FOR IBN
  * (c) 2024 by Nokia
 *
 * WARNING!!!
    ...
    for (neId in sitesConfigs) {
      ...
      if (result.success) {
        ...
        for (objectName in sitesConfigs[neId]) {
          // If 'value' is set, this must be a create, merge, or replace operation
          if (sitesConfigs[neId][objectName]['value']) {
